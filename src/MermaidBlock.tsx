...
const MermaidBlock: React.FC<MermaidProps> = ({
  ...
  useEffect(() => {
    ...
    const renderChart = async () => {
      ...
      try {
        const parse = await mermaidService.parse(code);
  ...
  return (
    ...
    >
      {/* JSX渲染方式 */}
      <div className="react-markdown-mermaid__instance" style={{ height: 300, border: '1px solid red', overflow: 'auto' }}>
        <h4>JSX渲染:</h4>
        {svgElement}
      </div>

      {/* 备用HTML渲染方式 */}
      <div style={{ height: 300, border: '1px solid blue', marginTop: '10px', overflow: 'auto' }}>
        <h4>HTML渲染:</h4>
